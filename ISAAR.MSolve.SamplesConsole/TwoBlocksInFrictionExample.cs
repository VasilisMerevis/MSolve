...
{
    public static class TwoBlocksInFrictionExample
    {
        ...
        public static void Two3DBlocksInFriction()
        {
            VectorExtensions.AssignTotalAffinityCount();
            double youngMod = 200e9;
            double poisson = 0.3;
            double verticalLoad = -200000000;
            double horizontalLoad = 20000;
            ...
            var element3 = new Element() { ID = 3, ElementType = new Contact3DNtSFr(material) };
            var element4 = new Element() { ID = 4, ElementType = new Contact3DNtSFr(material) };
            var element5 = new Element() { ID = 5, ElementType = new Contact3DNtSFr(material) };
            var element6 = new Element() { ID = 6, ElementType = new Contact3DNtSFr(material) };
            ...
            blocksModel.Loads.Add(new Load() { Amount = verticalLoad, Node = blocksModel.NodesDictionary[13], DOF = DOFType.Y });
            blocksModel.Loads.Add(new Load() { Amount = verticalLoad, Node = blocksModel.NodesDictionary[14], DOF = DOFType.Y });
            blocksModel.Loads.Add(new Load() { Amount = verticalLoad, Node = blocksModel.NodesDictionary[15], DOF = DOFType.Y });
            blocksModel.Loads.Add(new Load() { Amount = verticalLoad, Node = blocksModel.NodesDictionary[16], DOF = DOFType.Y });

            blocksModel.Loads.Add(new Load() { Amount = horizontalLoad, Node = blocksModel.NodesDictionary[10], DOF = DOFType.X });
            blocksModel.Loads.Add(new Load() { Amount = horizontalLoad, Node = blocksModel.NodesDictionary[11], DOF = DOFType.X });
            blocksModel.Loads.Add(new Load() { Amount = horizontalLoad, Node = blocksModel.NodesDictionary[14], DOF = DOFType.X });
            blocksModel.Loads.Add(new Load() { Amount = horizontalLoad, Node = blocksModel.NodesDictionary[15], DOF = DOFType.X });
            ...
            childAnalyzer.LogFactories[1] = new LinearAnalyzerLogFactory(new int[] {
                ...
                blocksModel.NodalDOFsDictionary[15][DOFType.Y] });

            parentAnalyzer.BuildMatrices();
            parentAnalyzer.Initialize();

            linearSolution.SubdomainsDictionary[1].Matrix.WriteToFile(@"c:\ESD\matrix");
            var lines = new List<string>();
            for (int i = 0; i < linearSolution.SubdomainsDictionary[1].Matrix.Rows; i++)
            {
                var line = String.Empty;
                for (int j = 0; j < linearSolution.SubdomainsDictionary[1].Matrix.Columns; j++)
                    line += linearSolution.SubdomainsDictionary[1].Matrix[i, j].ToString() + " ";
                line += "\n";
                lines.Add(line);
            }
            File.WriteAllLines(@"c:\ESD\matrix", lines);
